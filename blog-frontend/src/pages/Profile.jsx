...
const CompatBar = ({ score, badges }) => {
  if (!score && !badges?.length) return null;
  const color = score >= 70 ? '#22c55e' : score >= 40 ? '#3b82f6' : '#94a3b8';
  const label = score >= 70 ? '🔥 Très compatible' : score >= 40 ? '👍 Compatible' : '🌱 À découvrir';
  return (
    <div className="bg-gradient-to-r from-blue-50 to-indigo-50 border border-blue-100 rounded-2xl p-3 mt-3">
      <div className="flex items-center justify-between mb-2">
        <span className="text-xs font-bold text-blue-700">⚡ Compatibilité</span>
        <span className="text-xs font-bold" style={{ color }}>{label}</span>
      </div>
      <div className="h-2 bg-white rounded-full overflow-hidden border border-blue-100 mb-2">
      ...
      {badges?.length > 0 && (
        <div className="flex flex-wrap gap-1.5 mt-2">
          {badges.slice(0, 5).map((b, i) => (
            <span key={i} className="inline-flex items-center gap-1 bg-white border border-blue-100 text-blue-600 text-[10px] font-bold px-2 py-0.5 rounded-full">
              {b.emoji} {b.label}
            </span>
          ))}
        </div>
...
const MiniSuggestionCard = ({ user, onAdd }) => {
  ...
  return (
    <div className="flex items-center gap-3 p-3 bg-gray-50 rounded-2xl border border-gray-100 hover:border-blue-200 hover:bg-blue-50/30 transition">
      <Link to={`/profile/${user._id}`} className="relative shrink-0">
        <img src={avatarOf(user)} className="w-10 h-10 rounded-xl object-cover" alt="" />
        {user.isOnline && (
          <span className="absolute -bottom-0.5 -right-0.5 w-3 h-3 bg-emerald-500 border-2 border-white rounded-full" />
        )}
      </Link>
      <div className="flex-1 min-w-0">
        <Link to={`/profile/${user._id}`} className="font-bold text-xs text-gray-800 hover:text-blue-600 transition truncate block">
          {user.fullName}
        </Link>
        {user.jobTitle && <p className="text-[10px] text-gray-400 truncate">{user.jobTitle}</p>}
        {user.mainBadge && (
          <span className="inline-flex items-center gap-1 mt-0.5 bg-blue-100 text-blue-600 text-[9px] font-bold px-1.5 py-0.5 rounded-full">
            {user.mainBadge.emoji} {user.mainBadge.label}
          </span>
        )}
      </div>
      {status === 'added'
        ...
        : (
          <button onClick={handleAdd} disabled={status === 'adding'}
            className="shrink-0 bg-blue-600 text-white p-1.5 rounded-lg hover:bg-blue-700 transition disabled:opacity-50">
...
const CommonCircles = ({ userId }) => {
  const [circles, setCircles] = useState([]);
  const [loading, setLoading] = useState(true);
  useEffect(() => {
    API.get(`/circles/common/${userId}`)
  ...
  return (
    <div className="bg-white rounded-2xl shadow-sm p-4">
      <p className="text-xs font-bold text-gray-400 uppercase tracking-widest mb-3">
        ⭕ Cercles en commun ({circles.length})
      </p>
      <div className="flex flex-wrap gap-2">
        {circles.map(c => (
          <Link key={c._id} to={`/circles/${c._id}`}
            className="inline-flex items-center gap-1.5 bg-indigo-50 border border-indigo-100 text-indigo-700 text-xs font-bold px-3 py-1.5 rounded-full hover:bg-indigo-100 transition">
...
const RecommendModal = ({ userId, onClose }) => {
  ...
  useEffect(() => {
    API.get('/friends')
      .then(({ data }) => setFriends((data || []).filter(f => f._id !== userId)))
      .finally(() => setLoading(false));
  }, [userId]);
          ...
          {loading ? <Spinner /> : friends.length === 0
            ...
            : friends.map(f => (
              <button key={f._id} onClick={() => toggle(f._id)}
                className={`w-full flex items-center gap-3 px-3 py-2.5 rounded-xl transition ${sel.includes(f._id) ? 'bg-blue-50 ring-2 ring-blue-400' : 'hover:bg-gray-50'}`}>
    ...
    <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/60 p-4">
      <div className="bg-white rounded-2xl w-full max-w-lg shadow-2xl flex flex-col max-h-[92vh]">
        ...
        <div className="flex border-b shrink-0">
          {['general', 'pro', 'réalisations'].map(t => (
          ...
          {tab === 'general' && (
            <>
              <div>
                ...
                <label className="block relative h-24 rounded-xl overflow-hidden cursor-pointer group bg-gradient-to-br from-blue-400 to-purple-500">
                  {(newCover || profile.coverPhoto) && (
                    <img src={newCover ? URL.createObjectURL(newCover) : imgSrc(profile.coverPhoto)} className="w-full h-full object-cover" alt="" />
                  )}
              ...
              <div>
                ...
                <div className="flex items-center gap-4">
                  <img src={newAvatar ? URL.createObjectURL(newAvatar) : avatarOf(profile)} className="w-16 h-16 rounded-2xl object-cover border-2 border-gray-200" alt="" />
            ...
            <>
              {[
                ...
                <div key={f.k}>
                  ...
                  <input type="text" value={form[f.k]} placeholder={f.ph} onChange={e => set(f.k, e.target.value)}
                    className="w-full border border-gray-200 rounded-xl px-4 py-2.5 text-sm outline-none focus:ring-2 focus:ring-blue-400" />
                </div>
              ))}
              <div className="grid grid-cols-2 gap-3">
                {[{ k: 'yearsExp', l: "Années d'expérience" }, { k: 'totalProjects', l: 'Projets réalisés' }].map(f => (
                  <div key={f.k}>
                    <label className="text-xs font-bold text-gray-500 uppercase block mb-1">{f.l}</label>
              ...
              <div>
                ...
                <div className="grid grid-cols-3 gap-2">
                  {[{ v: 'open', l: '🟢 Open to work' }, { v: 'freelance', l: '🟡 Freelance' }, { v: 'unavailable', l: '⚫ Non dispo' }].map(opt => (
                    <button key={opt.v} onClick={() => set('availability', opt.v)}
                      className={`py-2 px-1 rounded-xl border-2 text-xs font-semibold text-center transition
                        ${form.availability === opt.v ? 'border-blue-500 bg-blue-50 text-blue-700' : 'border-gray-200 text-gray-600 hover:border-gray-300'}`}>
                      {opt.l}
                    </button>
                  ))}
                </div>
            ...
            <div className="space-y-3">
              {achievements.map((a, i) => (
                <div key={i} className="bg-gray-50 rounded-2xl p-3 space-y-2 border border-gray-100">
                  ...
                  <input value={a.description} placeholder="Description courte…" onChange={e => updAch(i, 'description', e.target.value)}
                    className="w-full border border-gray-200 rounded-xl px-3 py-1.5 text-sm outline-none" />
                  <input value={a.link} placeholder="Lien (optionnel)" onChange={e => updAch(i, 'link', e.target.value)}
                    className="w-full border border-gray-200 rounded-xl px-3 py-1.5 text-sm outline-none" />
                </div>
              ))}
              <button onClick={addAch}
                className="w-full py-3 border-2 border-dashed border-gray-300 rounded-2xl text-sm text-gray-500 hover:border-blue-400 hover:text-blue-500 transition flex items-center justify-center gap-2">
...
const Profile = () => {
  ...
  useEffect(() => {
    ...
    const load = async () => {
      try {
        ...
        setLiked(profileData.stats?.isLiked || false);
        setLikesCount(profileData.stats?.profileLikesCount || 0);

        try {
          const { data: artData } = isMe
        ...
        if (!isMe) {
          try {
            ...
            const found = (friendsData || []).find(f => f._id === id);
            if (found) setFriendship('accepted');
          } catch { /* silent */ }

          try {
            const { data: suggData } = await API.get('/users/suggestions?page=1');
            const match = suggData.suggestions?.find(s => s._id === id);
            if (match) setCompat({ score: match.normalScore, badges: match.badges });
          } catch { /* silent */ }
        }
      } catch (err) {
  ...
  const fetchSimilarUsers = useCallback(async () => {
    if (similarUsers.length > 0 || loadingSimilar) return;
    setLoadingSimilar(true);
    try {
      const { data } = await API.get('/users/suggestions?page=1');
      setSimilarUsers(
        (data.suggestions || []).filter(s => s._id !== id).slice(0, 4)
      );
    } catch { /* silent */ }
    finally { setLoadingSimilar(false); }
  }, [id, similarUsers.length, loadingSimilar]);
      ...
      <div className="space-y-4 pb-8">
        ...
        <div className="bg-white rounded-2xl shadow-sm overflow-hidden">
          ...
          <div className="px-5 pb-5">
                  ...
                  <>
                    <button onClick={handleLike} disabled={likeLoading}
                      className={`flex items-center gap-1.5 px-3 py-2 rounded-xl text-sm font-semibold border-2 transition
                        ${liked ? 'bg-rose-50 border-rose-300 text-rose-600' : 'bg-white border-gray-200 text-gray-600 hover:border-rose-300 hover:text-rose-500'}
                    ...
                    {friendship === 'accepted' ? (
                      ...
                      <button disabled className="px-3 py-2 rounded-xl text-sm font-semibold border-2 border-gray-200 text-gray-400">
                        Demande envoyée
                      </button>
                    ) : friendship !== 'blocked' ? (
            ...
            <div className="space-y-1">
              ...
              {!isMe && compat && <CompatBar score={compat.score} badges={compat.badges} />}
            </div>
          </div>
        </div>
        ...
        <div className="bg-white rounded-2xl shadow-sm p-4">
          ...
          <div className="flex flex-wrap gap-2">
            ...
            {recRate > 0 && (
              <div className="flex flex-col items-center px-4 py-3 rounded-2xl border bg-gradient-to-br from-emerald-50 to-teal-50 border-emerald-100 min-w-[72px]">
              ...
              <div>
                ...
                <div className="flex flex-wrap gap-2">
                  {profile.skills.map((s, i) => (
                    <span key={i} className="bg-blue-100 text-blue-700 text-xs font-bold px-3 py-1.5 rounded-full">{s}</span>
                  ))}
                </div>
              </div>
            )}
              ...
              <div>
                ...
                <div className="flex flex-wrap gap-2">
                  {profile.languages.map((l, i) => (
                    <span key={i} className="bg-violet-100 text-violet-700 text-xs font-bold px-3 py-1.5 rounded-full">{l}</span>
                  ))}
                </div>
              </div>
            )}
          ...
          <div className="bg-white rounded-2xl shadow-sm p-4">
            ...
            <div className="space-y-2">
              {profile.achievements.map((a, i) => (
                <div key={i} className="flex items-start gap-3 p-3 rounded-xl border border-gray-100 hover:border-blue-200 hover:bg-blue-50/30 transition group">
            ...
            <div className="divide-y divide-gray-50">
              {articles.length === 0 ? (
                ...
                <Link key={a._id} to={`/articles/${a._id}`}
                  ...
                  {a.coverImage && (
                    <img src={imgSrc(a.coverImage)} alt="" className="w-16 h-16 rounded-xl object-cover shrink-0" />
                  )}
                  <div className="flex-1 min-w-0">
                    <p className="font-bold text-gray-800 text-sm truncate group-hover:text-blue-600 transition">{a.title}</p>
                    <p className="text-xs text-gray-400 mt-0.5">
                      {new Date(a.createdAt).toLocaleDateString('fr-FR', { day: 'numeric', month: 'long', year: 'numeric' })}
                    </p>
                    <div className="flex items-center gap-4 mt-1.5 text-xs text-gray-400">
                      <span className="flex items-center gap-1"><EyeIcon className="w-3.5 h-3.5" /> {a.views || 0}</span>
                      <span className="flex items-center gap-1"><BookmarkIcon className="w-3.5 h-3.5" /> {a.savedBy?.length || 0}</span>
                      {a.tags?.slice(0, 2).map(tag => (
                        <span key={tag} className="bg-blue-50 text-blue-500 px-2 py-0.5 rounded-full">#{tag}</span>
                      ))}
                    </div>
                  </div>
                </Link>
              ...
              {!isMe && (
                <div className="flex gap-3">
                  ...
                  <div className="flex-1 space-y-2">
                    <textarea value={newComment} rows={2} onChange={e => setNewComment(e.target.value)}
                      placeholder={`Laisser une recommandation pour ${profile.fullName}…`}
                      className="w-full bg-gray-100 rounded-2xl px-4 py-2.5 text-sm outline-none resize-none focus:bg-gray-200 transition placeholder:text-gray-400" />
                    <button onClick={handleComment} disabled={!newComment.trim() || submitting}
                      className="bg-blue-600 text-white px-4 py-1.5 rounded-xl text-xs font-bold hover:bg-blue-700 disabled:opacity-40 transition">
                ...
                <div key={c._id} className="flex gap-3">
                  ...
                  <div className="flex-1 bg-gray-50 rounded-2xl px-4 py-3">
                    <div className="flex items-center justify-between mb-1">
                      <div className="flex items-center gap-2">
                        <Link to={`/profile/${c.author?._id}`} className="font-bold text-sm text-gray-800 hover:text-blue-600 transition">
                          {c.author?.fullName}
                        </Link>
                      ...
                      {(isMe || c.author?._id === me?._id) && (
                        <button onClick={() => handleDeleteComment(c._id)} className="text-gray-300 hover:text-red-400 transition ml-2">
                          <XMarkIcon className="w-4 h-4" />
                        </button>
                      )}
                    </div>
        ...
        {!isMe && (
          <div className="bg-white rounded-2xl shadow-sm p-4">
            <div className="flex items-center justify-between mb-3">
              <p className="text-xs font-bold text-gray-400 uppercase tracking-widest">👥 Vous aimerez aussi</p>
              {similarUsers.length === 0 && !loadingSimilar && (
                <button onClick={fetchSimilarUsers} className="text-xs text-blue-500 font-bold hover:underline">
            ...
            {loadingSimilar ? (
              ...
              <div className="space-y-2">
                {similarUsers.map(u => (
                  <MiniSuggestionCard key={u._id} user={u} onAdd={async (uid) => {
                    await API.post(`/friends/request/${uid}`);
                    toast.success('Demande envoyée !');
                  }} />
                ))}
                <Link to="/friends/suggestions" className="block text-center text-xs text-blue-500 font-bold mt-2 hover:underline">
                  Voir toutes les suggestions →
                </Link>
              </div>
            ) : (
              <p className="text-xs text-gray-400 text-center py-4">
                Cliquez sur "Voir les suggestions" pour découvrir des personnes similaires
              </p>
            )}
          </div>
        )}
