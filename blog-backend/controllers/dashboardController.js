...
const topArticlesRaw = [...myArticles]
  .sort((a, b) => (b.views + b.savedBy.length) - (a.views + a.savedBy.length))
  .slice(0, 5);

// Compter réactions et commentaires pour chaque article
const topIds = topArticlesRaw.map(a => a._id);

const [topComments, topReactions] = await Promise.all([
  Comment.aggregate([
    { $match: { article: { $in: topIds } } },
...
const topArticles = topArticlesRaw.map(a => ({
  ...
  shares:    a.shares || 0,
  date:      a.createdAt,
  comments:  topComments.find(c => c._id.toString() === a._id.toString())?.count  || 0,
  reactions: topReactions.find(r => r._id.toString() === a._id.toString())?.count || 0,
}));
